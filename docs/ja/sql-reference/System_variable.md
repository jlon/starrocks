...
* **説明**: 現在のセッションで Shield 権限のフォールバックに使用するアプリケーショングループを指定します。StarRocks は最初にユーザー個人の Shield 権限を確認します。個人権限が一致しない場合、ユーザーがログイン PSA の指定グループに所属することを確認してから、そのグループの権限だけを確認します。この変数が空の場合は、ユーザー個人の権限だけを確認します。
* **デフォルト**: ""
* **データタイプ**: String
* **導入バージョン**: v4.1.1
